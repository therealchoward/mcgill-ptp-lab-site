import type { Metadata } from "next";
import { PageHero } from "@/components/PageHero";
import { ScrollReveal } from "@/components/ScrollReveal";
import { SectionLabel } from "@/components/SectionLabel";
import { upcomingEvents, pastEvents, type Event } from "@/data/events";

export const metadata: Metadata = {
  title: "Events",
  description:
    "Upcoming and past workshops, conferences, and events hosted by the McGill PTP Lab.",
};

function EventCard({ event, delay }: { event: Event; delay: number }) {
  const tags = [event.host, event.access, event.location].filter(Boolean);

  return (
    <ScrollReveal delay={delay}>
      <article className="p-8 border border-border rounded-sm hover:-translate-y-[3px] hover:shadow-lg transition-all duration-300">
        <div className="flex flex-wrap items-center gap-3 mb-4">
          <span className="text-[13px] font-medium uppercase tracking-[0.1em] text-accent">
            {event.date ?? "Date to be announced"}
          </span>
          {tags.map((tag) => (
            <span
              key={tag}
              className="text-[12px] font-medium uppercase tracking-[0.1em] text-text-secondary px-2 py-0.5 border border-border rounded-sm"
            >
              {tag}
            </span>
          ))}
        </div>
        {event.series && (
          <p className="text-[13px] font-medium uppercase tracking-[0.1em] text-text-secondary mb-2">
            {event.series}
          </p>
        )}
        <h3 className="text-xl font-semibold">{event.title}</h3>
        {event.description && (
          <p
            className="mt-3 text-[15px] text-text-secondary leading-relaxed max-w-[50rem]"
            dangerouslySetInnerHTML={{ __html: event.description }}
          />
        )}
      </article>
    </ScrollReveal>
  );
}

export default function EventsPage() {
  return (
    <>
      <PageHero
        title="Events"
        subtitle="Workshops, talks, and conferences on the philosophy of technology."
      />

      <section className="px-6 pb-24 md:pb-32">
        <div className="max-w-[1200px] mx-auto">
          <ScrollReveal>
            <SectionLabel number="01" label="Upcoming" />
          </ScrollReveal>
          <div className="space-y-8">
            {upcomingEvents.map((event, i) => (
              <EventCard key={event.title} event={event} delay={i * 0.08} />
            ))}
          </div>

          <div className="my-16 h-px bg-border" />

          <ScrollReveal>
            <SectionLabel number="02" label="Past" />
          </ScrollReveal>
          <div className="space-y-8">
            {pastEvents.map((event, i) => (
              <EventCard key={event.title} event={event} delay={i * 0.08} />
            ))}
          </div>
        </div>
      </section>
    </>
  );
}
